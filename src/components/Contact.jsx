import React from "react";
import { Container } from "./common/container";
import { Button } from "./common/button";
import { Title } from "./common/title";

import "./contact.css";

const Contact = () => {
  return (
    <div id="contact" className="contact-area">
      <Container>
        <Title side="right" title="Contact" />
        <div className="contact">
          <div className="contact-status">
            <p>
              I am interested in working with any company that thinks my skill
              will be helpful for them. If you are looking for someone like me,
              please let me know. Or you can just 'say hi' to me.
            </p>
            <div>
              <Button
                title="Contact Me"
                mt="25px"
                bgColor="#00cf5d"
                color="#fff"
                link="mailto:samakshgrover@gmail.com"
              />
            </div>
          </div>
          <div className="contact-details">
            <ul>
              <li>
                <h5>Email</h5>
                <p>
                  samakshgrover@gmail.com <span>(Recommended)</span>
                </p>
              </li>
              <li>
                <h5>Whatsapp</h5>
                <p>
                  +91-8385930606 <span>(Always Available)</span>
                </p>
              </li>
              <li>
                <h5>Social</h5>
                <p>
                  Linkedin - www.linkedin.com/in/samakshgrover
                </p>
              </li>
              <li>
                <h5>Address</h5>
                <p>Sri Ganganagar, Rajasthan, India - 335001</p>
              </li>
            </ul>
          </div>
        </div>
      </Container>
    </div>
  );
};

export default Contact;
